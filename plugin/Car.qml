...
// The car, from svgrepo.com/svg/528173-family coupe (svgrepo.com/svg/127342).
//
// This replaces a hand-drawn Canvas silhouette. The reason that one existed --
// nerd-font car glyphs differ between patched families, so the one that looks
// right today is a picture of somebody else's car after the next theme change
// -- still holds, and a Shape satisfies it the same way a Canvas did: the
// geometry ships with the plugin and takes the theme colour.
//
// Drop-in for the old component: same `tint`, `running` and `spin`.
//
// THREE THINGS LEARNED BY RENDERING IT RATHER THAN ASSUMING.
//
// The source viewBox is square but the car occupies only y 34.69..64.69 of it,
// so used as-is the icon is a third of the height it could be. The Shape is
// translated up by that offset and the item's implicit aspect is the CONTENT's
// 3.313:1, which is also a better shape for a bar than a square.
//
// scale belongs on the ShapePath, not the item. Scaling the item tessellates
// the curves at source size and magnifies the result, so the roofline is
// visibly a polygon at panel size.
//
// And the wheels have spokes, so `spin` is worth keeping -- each wheel is its
// own Shape rotated about its own centre, which is the one thing a single path
// could not express.
Item {
  id: root
  ...
  // The source viewBox, and the band of it the car actually occupies.
  readonly property real vb: 99.382
  readonly property real inkY: 34.69
  readonly property real inkH: 29.99

  // Exposed so a call site that sets width can derive a height that matches
  // the artwork instead of guessing. The old glyph was square; this one is not,
  // and a square box would leave a short car floating in it.
  readonly property real aspect: vb / inkH
  ...
  NumberAnimation on phase {
    ...
    from: 0
    to: 360
    // Faster spin, shorter loop. Clamped so a motorway speed is not a strobe
    // at twelve pixels across.
